...
export function Hero(props: Props) {
  ...
  return (
    <div
      className={'flex items-center ml--10 mr--10 xl:ml-10 xl:mr--10 gap-20 relative ' +
        (isFirefox ? 'mr--10' : '')}
    >
      <img
          ...
          {props.categories && props.categories.length
            ? props.categories.map((r, i) => (
              <span
                ...
                key={r + '_' + i}
                onClick={() => navigate('/recipes', { state: { category: r } })}
              >
                {categoryLabel[r]}
              </span>
            ))
              ...
              {props.vetApprover
                ? (
                  <div
                    ...
                    onClick={() =>
                      window.open(props.vetApprover?.web, '_blank')}
                  >
